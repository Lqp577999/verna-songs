...
function render(){
 ...
 const visible=filtered.slice(page*50,(page+1)*50),visibleIds=new Set(visible.map(s=>s.id));
 ...
 for(const song of visible){
  ...
  const selectLabel=el('label',undefined,'row-select'),select=el('input');select.type='checkbox';select.checked=selected.has(song.id);select.disabled=busy;select.setAttribute('aria-label','选择 '+song.id+' '+song.title);
  select.onchange=()=>{select.checked?selected.add(song.id):selected.delete(song.id);render();$('#rows input[data-id="'+CSS.escape(song.id)+'"]')?.focus();};select.dataset.id=song.id;selectLabel.append(select);
  row.append(selectLabel,el('span',song.id,'number'),info,el('div',[song.language,song.genre,song.type].filter(Boolean).join(' · '),'row-meta'),actions);$('#rows').append(row);
 }
 ...
 $('#select-page').disabled=busy||!state||!visible.length;$('#select-page').checked=!!visible.length&&visible.every(s=>selected.has(s.id));$('#select-page').indeterminate=selected.size>0&&!$('#select-page').checked;
 $('#select-page').onchange=event=>{visible.forEach(s=>event.target.checked?selected.add(s.id):selected.delete(s.id));render();};
 $('#bulk').textContent=trash?'恢复所选':'移入回收站';$('#bulk').disabled=busy||!state||!selected.size;$('#selected-count').textContent='已选 '+selected.size+' 首';
...
async function refresh(){if(busy||!store)return false;busy=true;render();try{state=await store.load();selected.clear();preview=null;$('#import-summary').textContent='';$('#error').hidden=true;$('#sync').textContent='已读取仓库 · 第 '+state.revision+' 版';void checkPublication();return true;}catch(e){$('#error').hidden=false;$('#error').textContent=message(e);return false;}finally{busy=false;render();}}
async function save(op){if(busy||!store||!state)return false;busy=true;render();try{state=await store.save(op,state);selected.clear();$('#error').hidden=true;$('#notice').textContent='已确认保存到仓库。公开网站正在发布，请看下方状态；无需重复保存。';void checkPublication();return true;}finally{busy=false;render();}}
...
$('#bulk').onclick=async()=>{if(busy||!state||!selected.size)return;const ids=[...selected],type=trash?'restore':'remove';if(type==='remove'&&!confirm('将所选 '+ids.length+' 首歌曲移入回收站？之后可以恢复。'))return;try{await save({type,ids});}catch(e){$('#error').hidden=false;$('#error').textContent=message(e);}};
$('#search').oninput=()=>{selected.clear();page=0;render();};$('#first').onclick=()=>{selected.clear();page=0;render();};$('#last').onclick=()=>{selected.clear();page=Number.MAX_SAFE_INTEGER;render();};$('#prev').onclick=()=>{selected.clear();page--;render();};$('#next').onclick=()=>{selected.clear();page++;render();};$('#active-tab').onclick=()=>{selected.clear();trash=false;page=0;render();};$('#trash-tab').onclick=()=>{selected.clear();trash=true;page=0;render();};$('#add').onclick=()=>openEditor(null);$('#refresh').onclick=refresh;$('#check-published').onclick=()=>checkPublication();
